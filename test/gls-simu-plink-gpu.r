...
a_effect[1,]<-c( 1.04, 0.885, -2.055, 0.00);
a_effect[2,]<-c( 1.17, -0.20, 0.74,   0.00);
a_effect[3,]<-c( 1.40, -2.25, 1.00,   0.00);

d_effect[1,]<-c( 1.49, -2.135, 4.82,   0.00);
d_effect[2,]<-c( 1.045, 1.320, 1.905,  0.00);
d_effect[3,]<-c( 1.265, -1.225, 2.710, 0.00);

cov_effect[1,]<-c( 2.49, -1.135, 0.82, 0.00);
cov_effect[2,]<-c( -1.045, 2.320, 0.905,  0.00);


simu.data <- gls.simulate( phe.out, snp.out, simu_n= 1000, simu_grp=1, simu_p=1000, simu_snp_rho=0.4, simu_rho=0.1, simu_sigma2= 4,
...
ret1 <- gls.plink( phe.out, file.plink.bed, file.plink.bim, file.plink.fam, Y.prefix="Y", Z.prefix="Z", covar.names=c("X_1", "X_2"), fgwas.filter = F , gpu.used=T, options=list(nParallel.cpu=1, nLegendre=2) );
