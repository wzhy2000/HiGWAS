...
simu.data <- gls.simulate( phe.out, snp.out, simu_n= 5000, simu_grp=1, simu_p=2000, simu_snp_rho=0.4, simu_rho=0.1, simu_sigma2= 4,
...
ret1 <- gls.plink( phe.out, file.plink.bed, file.plink.bim, file.plink.fam, Y.prefix="Y", Z.prefix="Z", covar.names=c("X_1", "X_2"), fgwas.filter = F , gpu.used=T, options=list(nParallel.cpu=1) );
